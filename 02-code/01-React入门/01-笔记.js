 ...
 * render()
 * componentDidUpdate()
 * 3卸载:卸载组件 ReactDOM.unmountComponentAtNode()
 * componentWillUnMount()
 * 
 * react/vue中的key的作用/内部原理
 * 1. 虚拟DOM的key的作用
 *  1）key是虚拟DOM对象的标识,在更新显示的时候,key起着极其重要的作用
 *  2) 详细的说:当列表数组中的数据发生变化生成新的虚拟DOM后,React进行旧的Diff对比
 *    a. key没有变
 *    item数据没变,直接使用原来的真实DOM
 *    item数据变了,对原来的真实DOM进行数据更新
 *    b.key变了
 *    销毁原来真实DOM,根据item数据创建新的真实DOM显示(即使item数据没有变)
 *   [
 *    key=3,name='c',age,id
 *    key=1,name='a',age,id
 *    key=2,name='b',age,id
 ...
 * 
 * key为index问题
 *  1)添加/删除/排序==>产生没有必要的真实DOM更新,界面效果没问题,但是效率低
 ...
 *  内存中打包---->启动服务器
 * 生产环境打包:
 *  先内存中打包--->不启动服务器,本地生产打包文件
 * npm start 开发环境中打包
 * npm build 生产环境中打包
 * 
 * npm start--内存打包,启动服务器
 * 
 * 
 * 理解并使用async/await
 * 1.理解
